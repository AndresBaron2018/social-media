					...
					<form @submit.prevent="store" autocomplete="off">
						<div class="form-group row">
							...
							<div class="col-md-6">
								...
								<input type="text" class="form-control" placeholder="Name" v-model="user.document" required>
							</div>
							<div class="col-md-6">
								<label>Email</label>
							...
							<div class="col-md-6">
								...
								<select class="form-control" v-model="user.direction" required>
									<option value="">Choosee One</option>
...
<script>
	export default {
		...
		methods: {
			async store() {
				var url = "";
				if (this.type == "created") url = `/Seller/store`;
				if (this.type == "updated") url = `/Seller/update/${this.user.id}`;
